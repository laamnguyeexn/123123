...
export function DepartmentCalendar() {
  ...
  const departments = ['all', ...Array.from(new Set(meetings.map(m => m.department).filter(Boolean)))];
  ...
  const getMeetingsForDate = (day: number | null) => {
    ...
    return meetings.filter((meeting) => {
      const matchesDate = meeting.date === dateStr;
  ...
  const upcomingMeetings = meetings
    .filter((meeting) => {
      ...
      const matchesDepartment = selectedDepartment === 'all' || meeting.department === selectedDepartment;
      return meetingDate >= today && matchesDepartment;
    })
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-2xl font-semibold text-gray-900">Lịch phòng ban</h2>
          <p className="text-gray-600 text-sm mt-1">Xem tất cả cuộc họp theo phòng ban</p>
        </div>
                      ...
                      <div className="space-y-1">
                        {dayMeetings.slice(0, 2).map((meeting) => (
                          <div
                            ...
                            className="text-xs bg-green-100 text-green-700 px-2 py-1 rounded truncate hover:bg-green-200 transition-colors"
                            title={`${meeting.title} - ${meeting.department || 'Chung'}`}
                          >
                            {meeting.title}
                          </div>
                        ))}
